...
export default function VehiclesScreen({
  ...
  useEffect(() => {
    ...
    return;
  }, [showRefuel, showAddService, showFuelEntries, showServiceEntries, showAddVehicle, showReports, showTopList, openedFromEntries, openedFromServiceEntries, selectedFuelToEdit, selectedServiceToEdit]);
    ...
    <View style={theme.page}>
      ...
      {showHeaderMenu && (
        <View style={{ position: 'absolute', right: 18, top: 54, backgroundColor: '#fff', borderRadius: 8, padding: 8, shadowColor: '#000', shadowOpacity: 0.08, elevation: 6, zIndex: 100 }}>
          ...
          <TouchableOpacity accessibilityLabel="Ranking TOP10" style={{ paddingVertical: 8, paddingHorizontal: 12, flexDirection: 'row', alignItems: 'center' }} onPress={() => { setShowHeaderMenu(false); setShowTopList(true); }}>
            <Ionicons name="trophy-outline" size={18} color="#0B2545" style={{ marginRight: 8 }} />
            <Text style={{ fontWeight: '600' }}>Ranking TOP10</Text>
          </TouchableOpacity>
      ...
      {/* TopList modal-like screen */}
      {showTopList && (
        <View style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: '#fff', zIndex: 200 }}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', padding: 12, borderBottomWidth: 1, borderColor: '#eee' }}>
            <Text style={{ fontSize: 18, fontWeight: '600' }}>Ranking TOP10</Text>
            <TouchableOpacity onPress={() => setShowTopList(false)} style={{ padding: 8 }}>
              <Text style={{ color: '#2e86de' }}>Zamknij</Text>
            </TouchableOpacity>
          </View>
          {/* @ts-ignore */}
